...
const Renewed = () => {
    ...
    const exportData = useExportHook('RenewedClients.xlsx')
    let pageNumber = 0
    ...
    function getEnquiry() {
        axios.get(`${url}/memberForm/${pathVal}`, {
            ...
            .then((res) => {
                const data = res.data.filter((list) =>  findRenewedClient(list) ).reverse()
                setPrevData(data)
                setResult1(data)
                setOgList(data)
            })
        ...
        <CRow>
            <CCol lg={12} sm={12}>
                <CCard className='mb-3 border-top-success border-top-3'>
                    <CCardHeader>
                        <strong className="mt-2">Renewed Clients <span className='float-end'>Total Renewed Clients : 
