...
const handlePrint = useReactToPrint({
        ...
        axios.get(`${url}/employeeform/${id}`, {
            ...
            .then((res) => {
                if(res.status===200){
                    ...
                    setEmpDocumnet((prev)=>({...prev,MemBerId:id,empID:res.data.EmployeeID,empName:res.data.FullName}))
                }
    ...
    const getEmpDocData = ()=>{
         axios.get(`${url}/emplDocument/emp/${id}`,{headers}).then((res)=>{
         if(!res.status===200){
          return 
         }
      ...
      function toToggaleFrom(){
       setForm((prev=>!prev))
        ...
        setEmpDocumnet(prev=>({...prev,docName:'',docview:'',}))
      }
