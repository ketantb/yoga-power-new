...
import { useSelector } from 'react-redux';
  ...
  const GeneralProductTable = ({genralProduct,status,validate,toRenderErrorOnFrontend,clientReferance}) => {

    
  const [updatedItemId,setUPdatedImtemId] = useState([])
  const saveUpdate = useUpdateStock(`generelproduct`,setUPdatedImtemId)
  const updateStock = useDispatch()
  const createStockReport = useStockReport()

  const clearFunction = useSelector((el)=>el.genralProductDataClearFun)
  ...
  useEffect(()=>{
    if(updatedItemId.length){
      updateStock({type:'update genral Product',payload:updatedItemId.at(-1)})
      clearFunction()
    }
    ...
    return (
      <>
         {Boolean(genralProduct.length)&& <div>
                                  <h4>GENRAL PRODUCT</h4>
                                  </div>}
  
                            {Boolean(genralProduct.length)&& <CTable className='mt-3' align="middle" bordered style={{ borderColor: "#0B5345" }} hover responsive>
              ...
              <CTableRow className="text-center">
                             ...
                             <CTableDataCell colSpan={3}>{genralProduct.reduce((crr,el)=>crr+(el.item*el.productDetails.Product_Price),0)}</CTableDataCell>
               </CTableRow>
