...
const AuravedaTotalTable = ({auravedaStore,status,validate,toRenderErrorOnFrontend,clientReferance}) => {


  const [updatedItemId,setUPdatedImtemId] = useState([])
  const saveUpdate = useUpdateStock(`ayurvedamedicine`,setUPdatedImtemId)
  const updateStock = useDispatch()
  const createStockReport = useStockReport()
  const clearFunction = useSelector((el)=>el.auravedaStockDataClearFun)
  ...
  useEffect(()=>{
    if(updatedItemId.length){
      updateStock({type:'update Aurveda Product',payload:updatedItemId.at(-1)})
      clearFunction()
    }
  ...
  return (
    <>
         {Boolean(auravedaStore.length)&& <div>
                                <h4>AURAVEDA  PRODUCT</h4>
                                </div>}

                          {Boolean(auravedaStore.length)&& <CTable className='mt-3' align="middle" bordered style={{ borderColor: "#0B5345" }} hover responsive>
            ...
            <CTableRow className="text-center">
                           <CTableDataCell style={{ backgroundColor: "#0B5345", color: "white" }} colSpan={5} >Total</CTableDataCell>
                          <CTableDataCell colSpan={3}>{auravedaStore.reduce((crr,el)=>crr+(el.item*el.productDetails.Product_Price),0)}</CTableDataCell>
             </CTableRow>
...
export default AuravedaTotalTable
