...
const GeneralProduct = () => {
    ...
    useEffect(()=>{
        dispatch({type:'add genral Product', payload:{item:noofProduct,fun:clearFunction}})
    ...
    function toFilterData(data){
        ...
        return data.filter((el)=>{
            return((el.productCode+"").toLowerCase()||'').includes(searchFilter.search2.toLowerCase().trim())&&
            ...
            ((Math.abs(+el.soldQuantity)+"").includes(searchFilter.search8.trim())) &&
                ...
                <CTableRow>
                           <CTableDataCell >
                            ...
                            onChange={(e)=>setSearchFilter((prev)=>({...prev,search9:e.target.value}))} /> </CTableDataCell>     
                                 <CTableDataCell ><CFormInput disabled className='min-width-90' value={searchFilter.search10} 
